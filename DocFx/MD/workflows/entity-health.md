...
- **Entity Died Event**: Event raised when the entity dies. See [EntityDiedContext](xref:ElectricDrill.AstraHealth.Events.Contexts.EntityDiedContext) API for more details on the context parameter. For owned entities, `EntityDiedContext.Performer` is resolved from the damage performer through the **Kill Credit Attribution** field — see [Attribution](package-configuration.md#attribution).
